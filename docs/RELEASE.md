...
The source is version **1.12.0 (build 19)**: 1.11.0 (build 18) plus the MCP helper `focus-studio-mcp`, through which Claude Code, Codex and other MCP clients record, edit and export in Focus Studio once the person approves them (GitHub issue #1). A recording an AI tool starts shows its countdown, the tool's name, the sound it records and the time left in the recording control bar, even with no main window; its `duration` counts recorded time, so paused time is excluded. The universal candidate is `dist/candidates/1.12.0/Focus Studio.app`, packaged as `dist/releases/Focus-Studio-1.12.0-universal-local.dmg` and `.zip`. Verification for this version is in [VALIDATION-1.12.0.md](VALIDATION-1.12.0.md), including live checks from [MCP-QA.md](MCP-QA.md) on the `c493291` candidate: a stdlib Python MCP client over the bundled helper (background launch and approval, status and sources, a recording edited and exported, the sound prompt), then one-click connect and a real Claude Code session that recorded, zoomed and exported. That check found macOS's first microphone prompt appearing after the countdown; the fix (the prompt now comes before the countdown, and an AI tool's recording is refused when microphone access is off) is in the rebuilt candidate and covered by automated tests, not yet by a live run. Codex, MCP-QA sections 1 and 3–7, parts of section 2, and several recording paths (pause/resume, the other sound-prompt answers, cancelling, `stop_recording`, background music) were not checked live (listed there). Automated or package checks do not by themselves prove live recording. Do not overwrite or stop the currently installed app while preparing a candidate.
