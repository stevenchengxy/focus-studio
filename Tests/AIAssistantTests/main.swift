...
struct AIAssistantTests {
    ...
    static func main() async throws {
        ...
        step("recording sessions: returns once live, duration, wait_for_recording, cancel, sound prompt, microphone access"); try await recordingSessions(root: root)
    ...
    final class FakeApp: AppControlling {
        ...
        /// The person's Pause, as the control bar sets it.
        var isRecordingPaused = false
        /// Recorded time left while paused (it does not count down then).
        var pausedRemaining: TimeInterval?
        var recordingRemaining: TimeInterval? {
            guard recordingPhase == .recording else { return nil }
            if isRecordingPaused { return pausedRemaining }
            guard let autoStopAt = recordingSession?.autoStopAt else { return nil }
